...
OUTPUT_FILE="$OUTPUT_DIR/delete-user-account-pytest-$TIMESTAMP.txt"

mkdir -p "$OUTPUT_DIR"

{
  echo "Use Case #7 pytest run: Delete User Account"
  echo "Started: $(date)"
  echo "API_BASE_URL: ${API_BASE_URL:-http://localhost:8080}"
  echo ""
} | tee "$OUTPUT_FILE"

"$PYTHON" -m pytest -vv "$SCRIPT_DIR/test_delete_user_account_pytest.py" 2>&1 | tee -a "$OUTPUT_FILE"
