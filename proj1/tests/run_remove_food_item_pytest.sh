...
OUTPUT_FILE="$OUTPUT_DIR/remove-food-item-pytest-$TIMESTAMP.txt"

mkdir -p "$OUTPUT_DIR"

{
  echo "Use Case #9 pytest run: Remove a Food Item from the Menu"
  echo "Started: $(date)"
  echo "API_BASE_URL: ${API_BASE_URL:-http://localhost:8080}"
  echo ""
} | tee "$OUTPUT_FILE"

"$PYTHON" -m pytest -vv "$SCRIPT_DIR/test_remove_food_item_pytest.py" 2>&1 | tee -a "$OUTPUT_FILE"
