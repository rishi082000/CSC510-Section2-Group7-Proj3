...
OUTPUT_FILE="$OUTPUT_DIR/update-order-status-pytest-$TIMESTAMP.txt"

mkdir -p "$OUTPUT_DIR"

{
  echo "Use Case #10 pytest run: Update Order Status (Pick Up / Deliver)"
  echo "Started: $(date)"
  echo "API_BASE_URL: ${API_BASE_URL:-http://localhost:8080}"
  echo ""
} | tee "$OUTPUT_FILE"

"$PYTHON" -m pytest -vv "$SCRIPT_DIR/test_update_order_status_pytest.py" 2>&1 | tee -a "$OUTPUT_FILE"
