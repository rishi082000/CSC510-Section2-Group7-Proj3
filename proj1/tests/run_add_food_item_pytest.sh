...
OUTPUT_FILE="$OUTPUT_DIR/add-food-item-pytest-$TIMESTAMP.txt"

mkdir -p "$OUTPUT_DIR"

{
  echo "Use Case #8 pytest run: Add a Food Item"
  echo "Started: $(date)"
  echo "API_BASE_URL: ${API_BASE_URL:-http://localhost:8080}"
  echo ""
} | tee "$OUTPUT_FILE"

"$PYTHON" -m pytest -vv "$SCRIPT_DIR/test_add_food_item_pytest.py" 2>&1 | tee -a "$OUTPUT_FILE"
